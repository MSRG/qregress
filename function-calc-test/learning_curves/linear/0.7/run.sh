...
apptainer run -C -B ${cwd} /home/j/jacobsen/gjones/deb.sif /opt/miniconda/bin/python ${cwd}/main.py --save_path ${path}  --settings ${path}/${name}.json --train_set ${cwd}/${errorname}_linear_train.bin --test_set ${cwd}/linear_test.bin --scaler ${cwd}/linear_scaler.bin >> ${name}.out 2>&1
