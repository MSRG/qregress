...
ddirname=$(dirname `pwd` | xargs -n 1 basename)
echo $cwd
for i in */; do
    name=${i%/}
...
#! /bin/bash
#SBATCH -t 7-00:00:00
#SBATCH -J ${ddirname}_${errorname}_0.8_${name}
#SBATCH -N 1
#SBATCH -n 12
...
apptainer run -C -B ${cwd} /home/gjones/projects/def-jacobsen/gjones/deb.sif /opt/miniconda/bin/python ${cwd}/main.py --save_path ${path}  --settings ${path}/${name}.json --train_set ${cwd}/quadratic_train.bin --test_set ${cwd}/quadratic_test.bin --scaler ${cwd}/quadratic_scaler.bin >> ${name}.out 2>&1 
