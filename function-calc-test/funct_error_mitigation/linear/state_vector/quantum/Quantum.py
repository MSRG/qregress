...
class QuantumRegressor:
    ...
    def _cost(self, parameters):
        ...
        if self._batch_size is not None and self.njobs is not None:
            ...
            base_cost = np.mean(joblib.Parallel(n_jobs=self.njobs//2,verbose=0)(joblib.delayed(mean_squared_error)(self.y[i], self.predict(self.x[i], params=parameters)) for i in tqdm(batch_partitions,desc=f"Cost (Batches {len(batch_partitions)} of size {self._batch_size})")))
        else:
