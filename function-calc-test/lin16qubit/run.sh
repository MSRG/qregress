...
for i in */; do
    ...
    path="/scratch/j/jacobsen/gjones/lin5qubits/${name}"
...
apptainer run -C -B /scratch/j/jacobsen/gjones/lin5qubits ~/deb.sif /opt/miniconda/bin/python /scratch/j/jacobsen/gjones/lin5qubits/main.py --save_path ${path}  --settings ${path}/${name}.json --train_set /linear_train.bin --test_set /linear_test.bin --scaler /linear_scaler.bin >> ${name}.out 2>&1 
