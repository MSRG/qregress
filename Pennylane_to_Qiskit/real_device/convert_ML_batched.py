...
# !{sys.executable} -m pip install --upgrade qiskit-machine-learning
...
with open('0.5_5_DDCC_train.bin','rb') as f:
    ddcc_train = joblib.load(f)

with open('0.5_5_DDCC_test.bin','rb') as f:
    ddcc_test = joblib.load(f)

with open('0.5_5_DDCC_scaler.bin','rb') as f:
...
def batched_pred(params, ansatz, hamiltonian, num_qubits, X, _backend, shots, resilience_level, n_jobs):
    ...
    for idx, (anz, pars) in enumerate(zip(mapped_circuits,tiled_params)):
        with Batch(backend=_backend) as batch:
            if 'fake' in _backend.backend_name:
                jobid = str(idx)
            else:
...
scores, y_test_pred, y_train_pred = evaluate(x0, qc, mapped_observables, num_qubits, n_jobs, _backend, X_train, y_train, X_test=X_test, y_test=y_test, plot = True, title= 'A2_HWE-CNOT',y_scaler=scaler, shots=1024.0, resilience_level=1)
