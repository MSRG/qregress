...
Our learning curves are generated varying the number of training points from 10-80\% of the total data, while fixing the number of test points to 20\% of the total dataset.
In Fig. \ref{fig:learningcurves}, we analyze the effects of the number of training points of the best 5 qubit BSE49 and DDCC circuits, M-M-CZ{\_}HWE-CNOT using a RUD and AL of 1 and A2{\_}HWE-CNOT using a RUD of 
Next, we use learning curves, where the number of training points is varied, while the number of training points is varied to 
For the BSE49 dataset, using 5 qubits, we found that M-M-CZ{\_}HWE-CNOT offers less overfit models, but offer poorer performance regarding the training and test set R$^{2}$s, as highlighted in Fig. \ref{fig:BSE5_learning_curves}.
...
When we analyze the effects of the number of training points on the DDCC dataset using the A2{\_}HWE-CNOT PQC, we found that 


A2{\_}HWE-CNOT has $3nvk$ trainable parameters

Minimum number of classical parameters is $n+1$
...
\begin{figure}[H]
	...
	\end{subfigure}
	\caption{Learning curves for the (a) BSE49 database using the M-M-CZ{\_}HWE-CNOT circuit and (b) the DDCC dataset using the A2{\_}HWE-CNOT circuit. The x-axis denotes the percentage of training points, while the test set is held to 20\% of the total data. The y-axis denotes the R$^{2}$ of the training and test learning curves. Inlay plots show the training and test data for the classical models examined, where the bottom error bar denotes the minimum value, line the mean value, and top error bar the maximum value.}
...
\begin{table}[h!]
	...
	\begin{tabular}{|c|c|c|c|}
		...
		\hline
	\end{tabular}
	\caption{Comparison of models with respect to parameter count, two-qubit gate count, and circuit depth.}
\end{table}
...
PQCs for the BSE49 and DDCC datasets do not exhibit an advantage concerning model accuracy, the number of training points, or time to train the models
...
While the physics-based feature set of DDCC outperforms the structure-based feature set of BSE49, DDCC requires too many device executions to be useful on the current generation of devices
