...
This dataset consists of 4394 datapoints (!VIKI! paper says 4502), 1951 of which are existing and 2443 are hypthoetical structures, with 49 unique A-B single bond types.
...
An important aspect of machine learning in chemistry is the choice of molecular representation, or how the molecule is represented in the machine learning models.\cite{jones_molecular_2023}
 ...
 Cite:
DScribe\cite{himanen_dscribe_2020} CMs and SOAPs + REMatchKernel 

Ripser\cite{tralie_ripserpy_2018}
RDKit\cite{noauthor_rdkit_nodate}


SOAPs\cite{de_comparing_2016} were excluded due to the cost of computing the kernel 


To find the optimal molecular representation, we examined five different molecular representations including Coulomb Matrices \cite{rupp_fast_2012}, Molecular ACCess Systems fingerprint (MACCS)\cite{durant_reoptimization_2002}, persistence images (PIs) \cite{adams_persistence_2017,townsend_representation_2020,schiff_augmenting_2022}, RDKit [cite], and Morgan or extended-connectivity fingerprints \cite{morgan_generation_1965,rogers_extended-connectivity_2010}.



\begin{equation}
	X_{\text{RDKit},i} = X_{\text{RDKit},i}^{\text{R}} - X_{\text{RDKit},i}^{\text{R}}
	\label{eq:descriptor_garcia}
\end{equation}


An additional challenge for choosing a molecular representions for the BSE49 dataset is how to represent the overall chemical system


 We found that the best molecular representation across all models test, as shown in Fig. \ref{fig:classicalfuncfit}, was Morgan fingerprints using the \textit{sub} formulation.\cite{garcia-andrade_barrier_2023}
 

For each representation we tested two different choices of representation, one based on the the products minus the reactants, denoted by \textit{sub}, similar to the method used in [cite], and one based on just the reactants, denoted as \textit{AB}.


We use the same 80\% train/20\% test split as we use throughout our studies. 


One challenge of applying classical molecular representations for quantum machine learning models is mapping the classical features, often containing hundreds or thousands of features per sample, to the number of qubits used.
...
The next chemical dataset explored is based on the Data-Driven Coupled-Cluster (DDCC) methods found in Ref. \cite{townsend_data-driven_2019,jones_chapter_2023}.
