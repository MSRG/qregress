...
All state-vector simulations with 5 qubits were optimized using 1000 iterations, while 16 qubit simulations utilized early stopping due to computational cost considerations.
...
%old A1-A1-CNOT_Full-CRX (0.19/0.20)
...
The training set R$^2$ values range from a minimum of -1.4673 to a maximum of 0.1566, with a mean of -0.2131 and a standard deviation of 0.3463. For the test set, the R$^2$ values range from a minimum of -1.4774 to a maximum of 0.1740, with a mean of -0.2031 and a standard deviation of 0.3475.
...
On average, the best encoding layer is M-M-CNOT with a training and test R$^{2}$s of -0.0242 and -0.0229, respectively.
The best ansatz layer, is HWE-CNOT with an average training  R$^{2}$ of 0.0360 and test R$^{2}$ of 0.0473.
Overall, for the BSE49 dataset using 5 qubits, the best encoder-ansatz pair is M-M-CZ{\_}HWE-CNOT with a training set R$^{2}$ of 0.1566 and test set R$^{2}$ of 0.1740.



[This is bad; the 70 removed were originally ran with 250 iter... not 1000 like the rest...]
Following this analysis, 70 PQCs were removed based on performance regarding the computational costs of state-vector simulation and model accuracy.


Using the remaining 98 PQCs, we perform a similar analysis using 16 qubits.
...
pair A2{\_}Hadamard train R$^{2}$      0.0028 test R$^{2}$       0.0140 train MAE    19.8071 kcal/mol test MAE     20.0745 kcal/mol

Encoder A1 train R$^{2}$     -0.0501 test R$^{2}$      -0.0458 train MAE    20.7795 kcal/mol test MAE     21.3186 kcal/mol

variational HWE-CNOT train R$^{2}$     -0.0147 test R$^{2}$      -0.0025 train MAE    20.1601 kcal/mol test MAE     20.5422 kcal/mol
...
\end{itemize}
 ...
 Train R$^{2}$     0.6191
Test R$^{2}$      0.6184
train MAE    5.9739 $mE_{h}$ 
test MAE     5.9660 $mE_{h}$
pair: A2{\_}HWE-CNOT


 Train R$^{2}$     0.0163
Test R$^{2}$      0.0183
train MAE    12.7282 $mE_{h}$
test MAE    12.7116 $mE_{h}$
encoder: A1


 Train R$^{2}$     0.2817
Test R$^{2}$      0.2853
train MAE    11.6967 $mE_{h}$
test MAE    11.6871 $mE_{h}$
variational: HWE-CNOT
