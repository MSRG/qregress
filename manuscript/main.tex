...
\begin{figure}[H]
	...
	\caption{FIX:The performance of various molecular representations is shown using different classical machine learning models. \textit{AB} (top row) and \textit{sub} (bottom row) denote how the chemical reaction is represented. Each column denotes a different classical machine learning model, where the first column, denoted as knn, refers to \textit{k}-nearest neighbors, the second column, labeled rfr, denotes random forest regression, and krr, the last column, denotes kernel ridge regression. The x-axis of each sub-figure denotes the various molecular representations that are analyzed: Coulomb matrices (CM), Molecular ACCess Systems (MACCS), persistence images (PIs), RDKit fingerprints (RDKit), and Morgan fingerprints (Morgan).The y-axis of each sub-figure shows the performance of each model, evaluated using the R$^{2}$, where the training set is denoted using light blue and the test set is dark blue.}
...
For each two-electron excitation, $t^{ab}_{ij}$, a feature set can be derived based on the MP2 $t_{2}$-amplitudes, which are used to initialize the CCSD amplitudes.
The MP2 $t_{2}$-amplitudes take the following form,
\begin{equation}
...
In the feature set, the MP2 $t_{2}$-amplitudes are included, along with the numerator ($\mel{ij}{}{ab}$) and denominator ($\varepsilon_{i}+\varepsilon_{j}-\varepsilon_{a}-\varepsilon_{b}$), a binary feature to denote whether the excitation goes to the same virtual orbital, 


We generate a database consisting 199 water molecules with 31 features for each molecule, using Psi4\cite{parrish_psi4_2017} and Psi4Numpy\cite{smith_psi4numpy_2018}.
STO-3G\cite{hehre_selfconsistent_1970} basis set due for benchmarking reasons
