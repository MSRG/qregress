...
apptainer run -C -B ${cwd} /home/gjones/projects/def-jacobsen/gjones/deb.sif /opt/miniconda/bin/python ${cwd}/main.py --save_path ${path}  --settings ${path}/${name}.json --train_set ${cwd}/16_DDCC_train.bin --test_set ${cwd}/16_DDCC_test.bin --scaler ${cwd}/16_DDCC_scaler.bin >> ${name}.out 2>&1
