...
#! /bin/bash
#SBATCH -t 7-00:00:00
#SBATCH -J ${errorname}_Morgan_${name}
#SBATCH -N 1
#SBATCH -n 16
...
apptainer run -C -B ${cwd} /home/gjones/projects/def-jacobsen/gjones/deb.sif /opt/miniconda/bin/python ${cwd}/main.py --save_path ${path}  --settings ${path}/${name}.json --train_set ${cwd}/PCA5_Morgan_train.bin --test_set ${cwd}/PCA5_Morgan_test.bin --scaler ${cwd}/PCA5_Morgan_scaler.bin >> ${name}.out 2>&1
